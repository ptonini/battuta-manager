...
urlpatterns = [
    ...
    url(r'^(?P<node_type>(host|group))/(?P<node_name>[a-zA-Z0-9-.]+)/$',
        login_required(views.NodeDetailsView.as_view()), name='node_details'),

    url(r'^(?P<node_type>(host|group))/(?P<node_name>[a-zA-Z0-9-.]+)/vars/$',
        login_required(views.VariablesView.as_view()), name='variables'),

    url(r'^(?P<node_type>(host|group))/(?P<node_name>[a-zA-Z0-9-.]+)/(?P<relation>(parents|children|members))/$',
